...
class _SessionScreenState extends State<SessionScreen> {
  ...
  /// Tracks the pane's mouse-reporting state by watching for the
  /// DECSET/DECRST enable/disable sequences (xterm's own mouseHandler is
  /// always default-constructed, so it cannot gate the forwarding).
  void _trackMouseMode(PaneOutput output) {
    final before = _paneMouseEnabled[output.paneId] ?? false;
    if (output.line.contains('\x1b[?1000h') || output.line.contains('\x1b[?1006h')) {
      _paneMouseEnabled[output.paneId] = true;
    } else if (output.line.contains('\x1b[?1000l') || output.line.contains('\x1b[?1006l')) {
      _paneMouseEnabled[output.paneId] = false;
    }
    final after = _paneMouseEnabled[output.paneId] ?? false;
    if (after != before) {
      // xterm 4.0.0's parser does NOT wire DECSET 1000/1006 - set the
      // mouse mode ourselves so terminal.mouseInput produces SGR output.
      _terminals[output.paneId]
          ?.setMouseMode(after ? MouseMode.upDownScroll : MouseMode.none);
      if (mounted) {
        setState(() {});
      }
    }
  ...
  Terminal _ensurePane(String paneId) {
    ...
    _viewKeys[paneId] = GlobalKey<TerminalViewState>();
    final feeder = PaneOutputFeeder(
      client: _client,
      paneId: paneId,
      terminal: terminal,
  ...
  /// rest is sent as literal text - batched). SGR mouse sequences
  /// (generated by terminal.mouseInput from scroll gestures) are
  /// forwarded as `send-keys -M` mouse events instead.
  void _handleKeyboardInput(String paneId, String data) {
    if (data.startsWith('\x1b[<')) {
      _client.sendCommand(
          'send-keys -t $paneId -M ${_quote(data)}');
      return;
    }
    translateKeyboardInput(
      data,
                ...
                child: GestureDetector(
                  ...
                  child: AbsorbPointer(
                    ...
                    child: PaneView(
                      ...
                      viewKey: _viewKeys[_currentPane],
                      mouseEnabled: _paneMouseEnabled[_currentPane] ?? false,
                    ),
                  ),
                ),
              ),
